...
class MenuActivity : AppCompatActivity() , View.OnClickListener{
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if(requestCode == EVENT_CODE_ACTIVITY){
            ...
            if(resultCode == Activity.RESULT_OK){
                ...
                val age = data!!.getStringExtra("guestAge")
                val ageMonth = data!!.getStringExtra("guestAgeMonth")
                btnGuest.text = text
                pop(age.toInt(),ageMonth.toInt())
            }
        }else{
    ...
    private fun pop(age: Int, month: Int) {
        var alertDialog:CustomAlertDialog = CustomAlertDialog(this)
        alertDialog.setTitleandContent("Month is ${primaCheck(month)}",getResourceImage(age),"close")
        alertDialog.show()
    }
